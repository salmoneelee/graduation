...
int main() {
    ...
    parent_fission_bank[parent_neutron].direction = 0; 
    child_fission_bank[child_neutron].position = -1.0;
    child_fission_bank[child_neutron].direction = 0;
    ...
    for(int i = 0; i < N+1; i++) {
        fprintf(file2, "%dth neutron: %f %d\n", i+1, parent_fission_bank[i].position, parent_fission_bank[i].direction);
    }
