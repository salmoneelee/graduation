...
void remove_and_redistribute_neutrons(FILE *file4, neutron **child_fission_bank_double_pointer) {
    neutron *new_fission_bank = calloc(child_neutron + 5, sizeof(neutron)); 
    ...
    if (negative == 0) {
        free(neutron_position_data);
        return;
    }

    fprintf(file4, "before removal\n");
    for (int i = 0; i < num_bins; i++) {
        double number_of_neutrons = gsl_vector_get(neutron_position_data, i); 
        fprintf(file4, "bin_num: %d, number_of_neutrons: %f\n", i, number_of_neutrons);
    }
    fprintf(file4, "\n");
    fprintf(file4, "after removal\n");

    for (int i = 0; i < num_bins; i++) {
        double number_of_neutrons = gsl_vector_get(neutron_position_data, i); 
        while (number_of_neutrons < 0) {
            int k = i;
            double next_number_of_neutrons = 0;
            while (next_number_of_neutrons <= 0) {
                if (k == num_bins - 1){
                    k = i;
                    while (next_number_of_neutrons <= 0) {
                        if (k == -1) {
                            printf("Neutron removal failed.\n");
                            exit(EXIT_FAILURE);
                        }
                        else {
                            k--;
                            next_number_of_neutrons = gsl_vector_get(neutron_position_data, k);
                        }
                    }
                    break;
                }
            ...
            gsl_vector_set(neutron_position_data, k, next_number_of_neutrons - 1.0);
            gsl_vector_set(neutron_position_data, i, number_of_neutrons + 1.0);
            
            number_of_neutrons = gsl_vector_get(neutron_position_data, i);
        }
    }

    // redistribute the neutrons
    int temp_child_neutron = 0;
    for (int i = 0; i < num_bins; i++) {
        double number_of_neutrons = gsl_vector_get(neutron_position_data, i); 
        fprintf(file4, "bin_num: %d, number_of_neutrons: %f\n", i, number_of_neutrons);

        for (int j = 0; j < number_of_neutrons; j++) {
            temp_child_neutron++;
            new_fission_bank[temp_child_neutron].position = i * (width/num_bins) + (width/(2*num_bins)); // Mid-position of the bin
            new_fission_bank[temp_child_neutron].weight = 1.0;
        }
    }
    child_neutron = temp_child_neutron;
