...
int main() {
    ...
    const char *input = "input_5.txt"; // File for reading the inputs
    const char *filename1 = "flux_data_5.txt"; // File for storing neutron flux data
    const char *filename2 = "text_data_5.txt"; // File for storing data of each cycle
    const char *filename3 = "position_data_5.txt"; // File for storing neutron position data
    const char *filename4 = "sort_data_5.txt"; // File for storing sorted neutron data
    ...
    for(int i = 0; i < M; i++) {
        ...
        if (boundary_condition == 0) {
            // Log the position and weight of child neutrons before sort
            fprintf(file4, "----------- child before sort -----------\n");
            ...
            quicksort(child_fission_bank, 1, child_neutron);

            // Log the position and weight of child neutrons after sort
            fprintf(file4, "----------- child after sort-----------\n");
            for (int i = 1; i < child_neutron + 1; i++) {
                fprintf(file4, "%d %0.10f %0.10f\n", i, child_fission_bank[i].position, child_fission_bank[i].weight); 
            }

            // Remove positive weight neutrons using negative weight neutrons within the child fission bank
            remove_negative_weight(child_fission_bank_double_pointer);
