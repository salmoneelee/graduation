    ...
    for (int i = 0; i < chain_count; i++) {
        ...
        switch (chained_pointers[i].type) {
            ...
            case TYPE_GSL_VECTOR:
                ...
                break;
            case TYPE_GSL_MATRIX:
                gsl_matrix_free((gsl_matrix *)chained_pointers[i].data);
                break;
            case TYPE_RNG:
                free_rng();
                break;
...
gsl_vector *simulate_neutron_diffusion(FILE *file2, FILE *file5, neutron *parent_fission_bank, neutron **child_fission_bank_double_pointer, double *k_pointer, double k, gsl_matrix *A) {
    ...
    // Debug Start: Logging child positions
    fprintf(file5, "----------- child position vector after simulation -----------\n");
    double total_child_weight = 0.0;
    for (int i = 0; i < num_bins; i++) {
        fprintf(file5, "%d ~ %d: %.5f ", i, i+1, gsl_vector_get(child_position_vector, i));
        total_child_weight += gsl_vector_get(child_position_vector, i);
    }
    fprintf(file5, "\n"); 
    fprintf(file5, "total child weight: %f\n", total_child_weight); 
    // Debug End 

    unregister_pointer(child_position_vector);
    return flux;
} 
