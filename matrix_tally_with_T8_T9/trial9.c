...
int main() {
    ...
    const char *input = "input_9.txt"; // File for reading the inputs
    const char *filename1 = "flux_data_9.txt"; // File for storing neutron flux data
    const char *filename2 = "text_data_9.txt"; // File for storing data of each cycle
    const char *filename3 = "position_data_9.txt"; // File for storing neutron position data
    const char *filename4 = "reweight_bank_9.txt"; // File for storing reweight data of the child fission bank 
    const char *filename5 = "matrix_9.txt"; // File for storing fission matrix data
    const char *filename6 = "fission_matrix.txt"; // File for storing the final fission matrix data
    ...
    if(input_file == NULL || file1 == NULL || file2 == NULL || file3 == NULL || file4 == NULL || file5 == NULL || file6 == NULL) {
    ...
    for(int i = 0; i < M; i++) {
        ...
        // Log k values and initial position vector before simulation 
        fprintf(file5, "----------- cycle# = %d -----------\n", i+1);
        fprintf(file5, "----------- k before simulation -----------\n");
        fprintf(file5, "k: %f\n", k);
        fprintf(file5, "k_cycle: %f\n", k_cycle);
        fprintf(file5, "----------- initial position vector -----------\n");
        for (int i = 0; i < num_bins; i++) {{
            fprintf(file5, "%d ~ %d: %.5f ", i, i+1, gsl_vector_get(initial_position_vector, i));
        }
        fprintf(file5, "\n"); // Ensure new row
        }
        // Debug End
        ...
        gsl_vector *neutron_flux = simulate_neutron_diffusion(file2, file5, parent_fission_bank, child_fission_bank_double_pointer, k_pointer, k, A);
        ...
        // Log k values after simulation 
        fprintf(file5, "----------- k after simulation -----------\n");
        fprintf(file5, "k: %f\n", k);
        fprintf(file5, "k_cycle: %f\n", k_cycle);
        
        // Log matrix A and matrix A_sum after simulation
        double total_matrix_weight = 0.0;
        fprintf(file5, "----------- A after simulation -----------\n");
        for (size_t i = 0; i < A->size1; i++) {
            fprintf(file5, "row %zu: ", i);
            for (size_t j = 0; j < A->size2; j++) {
                fprintf(file5, "%.5f ", gsl_matrix_get(A, i, j));
                total_matrix_weight += gsl_matrix_get(A, i, j);
            }
            fprintf(file5, "\n"); 
        }
        fprintf(file5, "total matrix weight: %f\n", total_matrix_weight); 
        fprintf(file5, "----------- A_sum after simulation -----------\n");
        for (size_t i = 0; i < A_sum->size1; i++) {
            fprintf(file5, "row %zu: ", i);
            for (size_t j = 0; j < A_sum->size2; j++) {
                fprintf(file5, "%.5f ", gsl_matrix_get(A_sum, i, j));
            }
            fprintf(file5, "\n"); 
        }
    ...
    for (size_t i = 0; i < A_sum ->size1; i++) {
        for (size_t j = 0; j < A_sum ->size2; j++) {
            fprintf(file6, "%.5f ", gsl_matrix_get(A_sum, i, j));
