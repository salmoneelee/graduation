...
int main() {
    ...
    const char *input = "input_2.txt"; // File for reading the inputs
    const char *filename1 = "flux_data_2.txt"; // File for storing neutron flux data
    const char *filename2 = "text_data_2.txt"; // File for storing data of each cycle
    const char *filename3 = "position_data_2.txt"; // File for storing neutron position data
    ...
    if(input_file == NULL || file1 == NULL || file2 == NULL || file3 == NULL) {
    ...
    for(int i = 0; i < M; i++) {
        ...
        for (int i = 1; i < parent_neutron + 1; i++) {
            fprintf(file3, "%d %0.10f\n", i, parent_fission_bank[i].position); 
        }

        // Log total weight of parent neutrons before simulation
        fprintf(file2, "----------- cycle# = %d, size of fb = %d -----------\n", i+1, size_of_fb);
        fprintf(file2, "total number of neutrons = %d\n", parent_neutron);
        // Debug End

        // Simulate parent fission bank
        gsl_vector *neutron_flux = simulate_neutron_diffusion(file2, parent_fission_bank, child_fission_bank_double_pointer, k_pointer);
        ...
        k_cycle /= size_of_fb; // k value per neutron for a single cycle
        ...
        for (size_t i = 0; i < neutron_flux->size; i++) {
        ...
        for (int i = 1; i < child_neutron + 1; i++) {
            fprintf(file3, "%d %0.10f\n", i, child_fission_bank[i].position); 
        }
        // Debug End
        ...
        fission_bank_size_adjustment(child_fission_bank_double_pointer); 
