...
double determine_direction_and_distance(void) {
    ...
    double distance = -log(random_number_1)/(sigma_s + sigma_c + sigma_f);
    return distance * (2*random_number_2 - 1); // distance * cos(theta)
}
...
gsl_vector *simulate_neutron_diffusion(FILE *file2, neutron *parent_fission_bank, neutron **child_fission_bank_double_pointer, double *k_pointer) {
    ...
    while (parent_fission_bank[parent_neutron].position != -1.0) {
        ...
        fprintf(file2, "pos: %f, dis: %f, ",initial_position, distance);
        // Debug End
        ...
        if (new_position < width && new_position > 0) {
            ...
            sample_collision_type(file2, &fission_neutrons);
...
void sample_collision_type(FILE *file2, int *neutrons) {
    double random_number_1 = random_number_generator(); 
